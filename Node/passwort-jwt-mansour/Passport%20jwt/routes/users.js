...
router.get("/profile",
 ...
 router.post('/forgotPassword', async(req, res , next) => {
    ...
    and password confirmation to ${resetUrl} \n \n if you didn't forget your password please
     ignore this email and continue with your life like before. Have a nice day you beautiful
     human being.`
     try {
           await sendEmail({
         email:user.email,
 ...
 router.get('/resetPassword/:token' ,async (req , res , next) => {
   ...
   const hashrdToken = crypto.createHash('sha256').update(req.params.token)
   .digest('hex')
   const user = await User.findOne({passwordResetToken : hashrdToken
,passwordResetExpire:{$gt:Date.now()}})
if(!user){
    return next (new Error('Tokeb is invalid or has expired',400))
}


   // 2- if the token has expired and there is user , set now password
 // res.status(200).json({
  //  message:'Your token is Correect you can change your password'})
